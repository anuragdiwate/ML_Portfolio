...
vector<double> str2double(vector<string> v) {
  ...
  transform(v.begin(), v.end(), d.begin(),
...
train_naive_bayes(const vector<vector<double> > &train_data,
  ...
  vector<vector<double> > model_params = {
      means[0], stdevs[0], means[1], stdevs[1], {prior_0, prior_1}};
  return model_params;
}
...
vector<double>
evaluate_predictions(const vector<vector<double> > &test_data,
  ...
  double sensitivity =
      (double)num_true_positives / (num_true_positives + num_false_negatives);
  double specificity =
      (double)num_true_negatives / (num_true_negatives + num_false_positives);
  vector<double> metrics = {accuracy, sensitivity, specificity};
  return metrics;
}
...
int main() {
  ...
  int num_train = 800;
  vector<vector<double> > train_data(predictors.begin(),
                                    predictors.begin() + num_train);
  vector<double> train_y(response.begin(), response.begin() + num_train);
  vector<vector<double> > test_data(predictors.begin() + num_train,
                                   predictors.end());
  ...
  vector<double> metrics =
      evaluate_predictions(test_data, test_y, model_params);
  // output metrics
